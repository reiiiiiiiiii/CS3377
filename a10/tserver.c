	...
	while (1) {
		...
		while (fgets(line, MAXLEN, fp)) {
			...
			if (fork() == 0) {
				...
				//validate, score and send a response.
				...
				for(int i=0; i < reveal; i++) {
					int rindex = rand() % 5;
					starredWord[rindex] = selectedWord[rindex];
				}
			
				fprintf(clientfp, "Guess word for %s\n", selectedWord);
				fflush(clientfp);

				fgets(line, MAXLEN, serverfp);
				char *token = strtok(line, " \n");
				while (token) {
					if(!strcmp(token, selectedWord)) {
						fprintf(clientfp, "Correct. Word was %s\n", selectedWord);
						break;
					}
					token = strtok(NULL, " \n");
				}
				...
				if (token == NULL) 
					fprintf(clientfp, "Incorrect. Word was %s\n", selectedWord);
